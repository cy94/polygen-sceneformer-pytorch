...
from datasets.ply_dataset import PLYDataset, plydataset_collate_fn, get_num_classes
...
def run_training(cfg, subset, dev):
    ...
    if ckpt is not None:
        ...
        model = FaceModel(face_cfg, vtx_cfg, cfg["train"], num_classes=num_classes)
    print(f"Model parameters: {count_parameters(model)}")

    checkpoint_callback = ModelCheckpoint(save_last=True, save_top_k=5,)
    # specify the checkpoint again to get the Trainer params
    trainer = Trainer(
        gpus=1,
