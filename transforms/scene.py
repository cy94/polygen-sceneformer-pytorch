...
class Add_Flair_Embeddings():
    def __init__(self, emb_type='elmo', max_sentences=3, max_length=40):
        if emb_type == 'elmo':
            print('Using ELMO embedding')
            self.embedder = ELMoEmbeddings(embedding_mode='top')
        elif emb_type == 'bert':
            print('Using BERT embedding')
            self.embedder = TransformerWordEmbeddings('bert-base-cased', layers='-1')

        self.max_sentences = max_sentences
        self.max_length = max_length

    @staticmethod
    def embed_sentence(sentence, embedder, max_length):
        s = Sentence(sentence)
        embedder.embed(s)

        emb_dim = s[0].embedding.shape[0]
        n_pad = max_length - len(s)
        assert n_pad >= 0
        # pad with zeros
        pad = torch.ones((n_pad, emb_dim))

        emb = torch.cat([tok.embedding.unsqueeze(0).cpu() for tok in s] + [pad], dim=0)

        return torch.Tensor(emb)

    def __call__(self, sample):
        sentence = ''.join(sample['description'][:self.max_sentences])
        sample['desc_emb'] = self.embed_sentence(sentence, self.embedder, self.max_length)
...
class Add_Glove_Embeddings(object):
    ...
    @staticmethod
    def embed_sentence(sentence, glove, max_length):
        tokens = list(word_tokenize(sentence))
        # pad to maximum length
        tokens += ['<pad>'] * (max_length - len(tokens))
        emb = torch.cat([glove[token].unsqueeze(0) for token in tokens])
    ...
    def __call__(self, sample):
        sentence = ''.join(sample['description'][:self.max_sentences])
        # embed words
        sample['desc_emb'] = self.embed_sentence(sentence, self.glove, self.max_length)
...
class Augment_rotation(object):
    ...
    def __call__(self, sample):
        room = sample['room']
        floor = sample['floor']
        # # degree_list = [0, 30, 60, 90, 120, 150,  180, 210, 240, 270]
        # self.degree_list = [0 , 90, 180, 270]
...
class Add_Descriptions(object):
    ...
    def __call__(self, sample):
        ...
        obj_names = map(clean_obj_name, sample['cat_name_seq'])
        obj_names = filter(lambda n: n not in ('door', 'window',), obj_names)
        ...
        # randomize the first sentence beginning
        s = random.choice(["The room has ",
