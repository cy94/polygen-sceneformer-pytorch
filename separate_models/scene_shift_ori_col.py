...
class scene_transformer(LightningModule):
    ...
    def decode_test(self, cat_seq, x_seq, y_seq, z_seq):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        for out_ndx in range(generate_seq_len):
            ...
            out_ori = self.output_ori(out_embs)[out_ndx][0]

            logprobs_ori = F.log_softmax(out_ori, dim=-1)
            _, ori_next_token = torch.max(logprobs_ori, dim=0)

            cat_gen_seq.append(cat_seq[out_ndx + 1])
            orient_gen_seq.append(ori_next_token)
