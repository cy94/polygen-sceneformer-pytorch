...
class scene_transformer(LightningModule):
    ...
    def decode_test(self, cat_seq, x_seq, y_seq, z_seq, orient_seq):

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        generate_seq_len = self.decoder_seq_len - 1
        cat_start_seq = (
            torch.LongTensor([cat_seq[0]] + generate_seq_len * [0])
            .view(1, -1)
            .to(device)
        )
        x_start_seq = (
            torch.LongTensor([x_seq[0]] + generate_seq_len * [0]).view(1, -1).to(device)
        )
        y_start_seq = (
            torch.LongTensor([y_seq[0]] + generate_seq_len * [0]).view(1, -1).to(device)
        )
        z_start_seq = (
            torch.LongTensor([z_seq[0]] + generate_seq_len * [0]).view(1, -1).to(device)
        )
        orient_start_seq = (
            torch.LongTensor([orient_seq[0]] + generate_seq_len * [0])
            .view(1, -1)
            .to(device)
        )
        dim_start_seq = (
            torch.LongTensor(
                [self.cfg["model"]["dim"]["start_token"]] + generate_seq_len * [0]
            )
            .view(1, -1)
            .to(device)
        )
        ...
        cat_gen_seq = [cat_seq[0]]
        x_gen_seq = [x_seq[0]]
        y_gen_seq = [y_seq[0]]
        z_gen_seq = [z_seq[0]]
        orient_gen_seq = [orient_seq[0]]
        dim_gen_seq = [self.cfg["model"]["dim"]["start_token"]]

        for out_ndx in range(generate_seq_len):
            out_embs = self.generator(tgt, tgt_mask)  # , tgt_padding_mask)

            out_ori = self.output_dim(out_embs)[out_ndx][0]

            logprobs_dim = F.log_softmax(out_ori, dim=-1)
            _, dim_next_token = torch.max(logprobs_dim, dim=0)

            cat_gen_seq.append(cat_seq[out_ndx + 1])
            orient_gen_seq.append(orient_seq[out_ndx + 1])
            x_gen_seq.append(x_seq[out_ndx + 1])
            y_gen_seq.append(y_seq[out_ndx + 1])
            z_gen_seq.append(z_seq[out_ndx + 1])
            dim_gen_seq.append(dim_next_token)
