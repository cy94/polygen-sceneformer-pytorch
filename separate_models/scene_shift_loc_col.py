...
class scene_transformer(LightningModule):
    def __init__(self, cfg):
        ...
        if self.text_cond:
            ...
            self.text_emb_linear = nn.Sequential(
                    nn.Linear(768, 256),
                    nn.ReLU(),
                    nn.Dropout(p=0.2),
    ...
    def decode_test(self, cat_seq, orient_seq, cur_cat_seq):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        curr_cat_start_seq = (
            torch.LongTensor([cur_cat_seq[0]] + generate_seq_len * [0])
            .view(1, -1)
            .to(device)
        )
        ...
        (
            ...
            cat_start_seq, loc_start_seq, orient_start_seq, curr_cat_start_seq
        )
        joint_emb = cat_emb + pos_emb + loc_emb + ori_emb + coor_type_emb + curr_cat_emb
        ...
        loc_gen_seq = [self.cfg["model"]["coor"]["start_token"]]
        orient_gen_seq = [orient_seq[0]]
        curr_cat_gen_seq = [cur_cat_seq[0]]
        for out_ndx in range(generate_seq_len):
            out_embs = self.generator(tgt, tgt_mask)

            out_loc = self.output_loc(out_embs)[out_ndx][0]

            logprobs_loc = F.log_softmax(out_loc, dim=-1)
            _, loc_next_token = torch.max(logprobs_loc, dim=0)

            cat_gen_seq.append(cat_seq[out_ndx + 1])
            orient_gen_seq.append(orient_seq[out_ndx + 1])
            loc_gen_seq.append(loc_next_token)
            curr_cat_gen_seq.append(cur_cat_seq[out_ndx + 1])
            ...
            curr_curr_cat_seq = curr_cat_gen_seq + (
                self.decoder_seq_len - len(orient_gen_seq)
            ) * [0]
            curr_curr_cat_seq = (
                torch.LongTensor(curr_curr_cat_seq).view(1, -1).to(device)
            )
            ...
            joint_emb = (
                cat_emb + pos_emb + loc_emb + ori_emb + coor_type_emb + curr_cat_emb
            )
            tgt = joint_emb.transpose(1, 0)
